...
def play_cups_new(cups, current_cup, time):
    ...
    #        print(new_cups, index%len(cups), new_cups[index%len(cups)])
        ...
        # Fjern hånden ved at lime på begge sider
        cups[current_cup] = get_nthbour(cups, current_cup, 3)
        destination_cup = ((current_cup - 2) % (len(cups)-1)) + 1
        while destination_cup in hand:
            destination_cup = ((destination_cup - 2) % (len(cups)-1)) + 1
        # Lim hånd på destination+1
        try:
            cups[hand[-1]] = cups[destination_cup]
        except IndexError:
            print("IE")
            print(hand[-1], destination_cup, current_cup)
            quit()
        # Lim destination på hånd
        cups[destination_cup] = hand[0]
...
new_cups_list = (len(new_cups) + 1) * [0]
print(len(new_cups_list))
for cup, cup_right in new_fancy_cups.items():
    try:
        new_cups_list[cup] = cup_right
    except IndexError:
        print("IE")
        print(cup, cup_right)

#result_cups = play_cups_new(new_fancy_cups, cups[0], 10000000)
result_cups = play_cups_new(new_cups_list, cups[0], 10000000)
